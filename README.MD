...
Maximum speed at the moment is the speed of reading short bitsets from a file, for bitset <3> it is 27.7 MB/s with /O2 compiler argument (maximum optimization of speed). For bools it works only 30% slower than a usual fstream, but writes 8 times more information per one byte.
